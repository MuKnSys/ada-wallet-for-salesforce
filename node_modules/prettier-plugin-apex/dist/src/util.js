...
import { APEX_TYPES, DATA_CATEGORY, MODIFIER, ORDER, ORDER_NULL, QUERY, QUERY_WHERE, } from "./constants.js";
export function isBinaryish(node) {
    return (node["@class"] === APEX_TYPES.BOOLEAN_EXPRESSION ||
...
export function checkIfParentIsDottedExpression(path) {
    ...
    else if (node["@class"] === APEX_TYPES.VARIABLE_EXPRESSION &&
...
export function massageAstNode(ast, newObj) {
    ...
    if (ast.scope && typeof ast.scope === "string") {
        ...
        for (let i = newObj.expr.length - 1; i >= 0; i -= 1) {
            if (newObj.expr[i]["@class"] === APEX_TYPES.WHERE_COMPOUND_EXPRESSION &&
...
export async function getNativeExecutableWithFallback() {
    ...
    try {
        ...
        const nativeBin = nodePath.join(packageName, getNativeExecutableNameForPlatform(`${platform}-${arch}`));
        const require = module.createRequire(import.meta.url);
        return nodePath.relative(process.cwd(), require.resolve(nativeBin));
    }
    catch (e) {
        if ("code" in e && e.code === "MODULE_NOT_FOUND") {
            console.warn(`Your platform ${platform}-${arch} is natively supported by Prettier Apex, but the executable cannot be found.`);
            console.warn(`If you didn't intentionally install Prettier Apex with ignore-optional flag, please file a bug report.`);
            console.warn(`Falling back to Java-based serializer.`);
        }
        return nodePath.join(await getSerializerBinDirectory(), `apex-ast-serializer${process.platform === "win32" ? ".bat" : ""}`);
    }
}
