...
function generateCode(ast) {
    ...
    const gen = new generator.CodeGenerator(ast, BABEL_GENERATE_OPTIONS);
...
function generateUid(cache, name) {
    ...
    // backing cache.
    const baseName = stripUid(core.types.toIdentifier(name));
    let uid = baseName;
    let i = 1;
    while (cache.has(uid)) {
...
function collectGlobalAliases({ bindings }, { globals: { toIds } }) {
    ...
    for (let i = 0, { length } = bindingNames; i < length; i += 1) {
        ...
        if (core.types.isIdentifier(initNode)) {
            const { name } = initNode;
        ...
        getGlobalReplacement(globalName) {
            ...
            if (id) {
                ...
                if (idName !== globalName && !injected.has(idName)) {
                    ...
                    programScope.push({
                        id,
                        init: core.types.identifier(globalName),
                    });
                }
            }
            return id;
...
function resolveGlobalReplacements({ meta: { globals: fromPlaceholder } }, { getAllBindingNames, globals: { toIds, toReplacement } }) {
    Object.values(fromPlaceholder).forEach((globalName) => {
        ...
        if (ids === undefined) {
            ids = [core.types.identifier(globalName)];
...
const Builder = class Builder {
    ...
    { name = 'unnamed', globals = {}, validator = alwaysFalse } = {}) {
        const expression = core.template.expression(code);
        ...
        const { [name]: builder } = {
            ...
            core.types.parenthesizedExpression(expression(
...
module.exports = transformUnforgeables;
